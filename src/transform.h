...
#define CC2PB_DEADZONE 2       // CC units either side of 64 that map to centre
#endif
#ifndef CC2PB_RANGE_PCT
#define CC2PB_RANGE_PCT 100    // % of the full 14-bit bend used at the CC extremes
#endif
...
inline int apply(uint8_t *p, uint16_t len, uint8_t rangePct) {
  ...
  while (i < len) {
    ...
    i += 1 + blemidi::dataBytesFor(s);   // skip this message
  }
  return hits;
#endif
}
